...
Public Class Service
    ...
    Public Function GetExorLiveId(ByVal localId As Integer) As Entities.GetExorLiveIdResponse Implements IService.GetExorLiveId
        ...
        Dim _exorliveContact As New WSUserService.User() With { _
        ...
        Try
            _exorliveContact = Util.CallUserService(token, Function(client) client.CreateContact(Nothing, _exorliveContact)).Value

            If _exorliveContact.ErrorCode = WSUserService.User.UserErrorCode.EmailTaken Then
                response.Status = Entities.GetExorLiveIdResponse.StatusType.EmailTaken
                response.ErrorMessage = response.Status.ToString()
                Return response
            End If
            If _exorliveContact.ErrorCode <> WSUserService.User.UserErrorCode.NoError Then
    ...
    Public Function CreateExorLiveAccount() As Entities.CreateExorLiveAccountResponse Implements IService.CreateExorLiveAccount
        ...
        Dim localUser As Entities.User = Database.Instance.GetUser(HttpContext.Current.User.Identity.Name)
        Dim exorLiveUser As New WSPartnerService.User() With { _
            .Firstname = localUser.FullName, _
            .Lastname = localUser.FullName, _
            .Email = localUser.EMailAddress}

        Dim endpointAddress As ServiceModel.EndpointAddress = New ServiceModel.EndpointAddress(New Uri(ExorLiveConsumer.Settings.ServiceEndpointBase & "services/PartnerService2.svc"))
        Dim binding As Channels.Binding = New BasicHttpBinding(If(endpointAddress.Uri.Scheme = Uri.UriSchemeHttps, BasicHttpSecurityMode.Transport, BasicHttpSecurityMode.None))

        Using client As New WSPartnerService.PartnerServiceClient(binding, endpointAddress)
            Try
                exorLiveUser = client.CreateOrganization(New WSPartnerService.Credentials() With _
                                                       {.Identification = Util.ConsumerKey, .Secret = Util.ConsumerSecret}, _
                                                       exorLiveUser, OpenID.Util.BuildIdentityUrl(localUser.Username).ToString()).Value
            Catch ex As Exception
                response.Status = Entities.CreateExorLiveAccountResponse.StatusType.Error
                response.ErrorMessage = ex.Message
            Finally
                client.Close()
            End Try
            If exorLiveUser.ErrorCode = WSPartnerService.User.UserErrorCode.NoError Then
